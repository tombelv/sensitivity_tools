...
class Model:
    ...
    def integrate_rk4(self, state, inputs, params, ext=0):
        ...
        k2 = self.dynamics_parametric(state + k1 * self.dt / 2.0, inputs, params, ext)
        k3 = self.dynamics_parametric(state + k2 * self.dt / 2.0, inputs, params, ext)
    ...
    def integrate_si_euler(self, state, inputs, params, ext=0):
        ...
        v_kp1 = (
            state[self.nq :]
            + self.dt * self.dynamics_parametric(state, inputs, params, ext)[self.nq :]
        )
        return cs.vertcat(
            state[: self.nq]
            + self.dt
            * self.dynamics_parametric(cs.vertcat(state[: self.nq], v_kp1), inputs, params, ext)[
                : self.nq
            ],
            v_kp1,
        )
    ...
    def sensitivity_step(self, state, inputs, state_sensitivity, input_gains=0, ext=0):
        p_sens_state, p_sens_inputs, p_sens_params = self.partial_sens_all(
            state, inputs, self.nominal_parameters, ext
        )
