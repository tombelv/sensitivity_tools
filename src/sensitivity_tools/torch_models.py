...
class BaseModel(ABC):
    ...
    def __init__(self, nq: int, nv: int, nu: int, np: int = 0, 
    ...
    @abstractmethod
    def sensitivity_step(self, state: Any, inputs: Any, params: Any, 
                        state_sensitivity: Any, input_sensitivity: Any, 
                        dt: float) -> Any:
        pass
...
class ModelParametric(BaseModel):
    ...
    def integrate_si_euler(self, state: Any, inputs: Any, params: Any, dt: float) -> Any:
        ...
        q_kp1 = state[:self.nq] + dt * self.dynamics_parametric(
            torch.cat([state[:self.nq], v_kp1]), inputs, params)[:self.nq]
...
class Model(ModelParametric):
    ...
    def __init__(self, model_dynamics,
        ...
        """
        super().__init__(model_dynamics, nq, nv, nu, len(nominal_parameters), 
                        input_bounds, integrator_type, device=device)
...
def ellipsoid_radius_torch(sensitivity: Any, scaling_matrix: Any, 
                          direction: Optional[Any] = None) -> Any:
    """
